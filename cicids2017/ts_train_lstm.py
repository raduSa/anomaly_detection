...
if __name__ == '__main__':
    ...
    print(f'Training Predictive LSTM on {X_train.shape[0]} windows â€¦')
    model, train_losses, val_losses = train(X_train, device)

    torch.save(model.state_dict(), os.path.join(MODELS_DIR, 'lstm_cicids_temporal.pt'))

    scores = reconstruction_scores(model, X_test, device)

    threshold = sweep_threshold(y_test, scores, beta=2.0)
    y_pred = evaluate(y_test, scores, threshold, 'Predictive LSTM (CIC-IDS2017 temporal)',
                      out_path=os.path.join(RESULTS_DIR, 'lstm_cicids_temporal_report.txt'))
    plot_results(y_test, y_pred, scores, 'Predictive LSTM (CIC-IDS2017 temporal)',
                 out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'cicids', 'lstm_cicids_temporal_results.png'))
    plot_loss_curves(train_losses, val_losses,
                     out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'cicids', 'lstm_loss_curve.png'))
    ...
    evaluate_per_class(attack_types, scores, skip_labels=SKIP_LABELS,
                       out_path=os.path.join(RESULTS_DIR, 'lstm_cicids_temporal_perclass_ap.txt'))
