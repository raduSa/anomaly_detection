...
if __name__ == '__main__':
    ...
    torch.save(model.state_dict(), os.path.join(MODELS_DIR, 'transformer_bottleneck_cicids_temporal.pt'))

    scores = reconstruction_scores(model, X_test, device)

    threshold = sweep_threshold(y_test, scores, beta=2.0)
    y_pred = evaluate(y_test, scores, threshold, 'Transformer Bottleneck AE (CIC-IDS2017 temporal)',
                      out_path=os.path.join(RESULTS_DIR, 'transformer_bottleneck_cicids_temporal_report.txt'))
    plot_results(y_test, y_pred, scores, 'Transformer Bottleneck AE (CIC-IDS2017 temporal)',
                 out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'cicids', 'transformer_bottleneck_cicids_temporal_results.png'))
    plot_loss_curves(train_losses, val_losses,
                     out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'cicids', 'transformer_bottleneck_loss_curve.png'))
    ...
    evaluate_per_class(attack_types, scores, skip_labels=SKIP_LABELS,
                       out_path=os.path.join(RESULTS_DIR, 'transformer_bottleneck_cicids_temporal_perclass_ap.txt'))
