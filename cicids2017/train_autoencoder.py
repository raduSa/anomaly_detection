...
BATCH_SIZE = 2048
LR = 1e-3
# Encoder: input â†’ 64 â†’ 32 â†’ 16 (bottleneck)
# Decoder: 16 â†’ 32 â†’ 64 â†’ input
HIDDEN = [64, 32, 16]
DROPOUT = 0.1
...
class MLP_AE(nn.Module):
    def __init__(self, input_dim, hidden):
        ...
        for i in range(len(dec_dims) - 1):
            if i < len(dec_dims) - 2:
                decoder_layers += [nn.Linear(dec_dims[i], dec_dims[i + 1]), nn.ReLU()]
                decoder_layers.append(nn.Dropout(DROPOUT))
            else:
                # Final layer: sigmoid keeps output in [0, 1] to match MinMax-scaled input.
                decoder_layers += [nn.Linear(dec_dims[i], dec_dims[i + 1]), nn.Sigmoid()]
...
if __name__ == '__main__':
    ...
    print(f'Training MLP AE on {X_train.shape[0]} samples, {X_train.shape[1]} features â€¦')
    ...
    y_pred = evaluate(y_test, scores, threshold, 'MLP AE (CIC-IDS2017)',
                      out_path=f'{RESULTS_DIR}/autoencoder_report.txt')
    plot_results(y_test, y_pred, scores, 'MLP AE (CIC-IDS2017)',
                 out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'cicids', 'autoencoder_cicids_results.png'))
    plot_loss_curves(train_losses, val_losses,
                     out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'cicids', 'autoencoder_loss_curve.png'))
    ...
    evaluate_per_class(attack_types, scores, skip_labels=SKIP_LABELS,
                       out_path=f'{RESULTS_DIR}/autoencoder_perclass_ap.txt')
