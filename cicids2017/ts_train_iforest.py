...
if __name__ == '__main__':
    ...
    joblib.dump(model, os.path.join(MODELS_DIR, 'iforest_cicids_temporal.pkl'))

    scores = -model.score_samples(X_test)

    threshold = sweep_threshold(y_test, scores, beta=2.0)
    y_pred = evaluate(y_test, scores, threshold, 'Isolation Forest (CIC-IDS2017 temporal)',
                      out_path=os.path.join(RESULTS_DIR, 'iforest_cicids_temporal_report.txt'))
    plot_results(y_test, y_pred, scores, 'Isolation Forest (CIC-IDS2017 temporal)',
                 out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'cicids', 'iforest_cicids_temporal_results.png'))
    ...
    evaluate_per_class(attack_types, scores, skip_labels=SKIP_LABELS,
                       out_path=os.path.join(RESULTS_DIR, 'iforest_cicids_temporal_perclass_ap.txt'))
