...
if __name__ == '__main__':
    ...
    torch.save(model.state_dict(), f'{MODELS_DIR}/lstm_{BENCHMARK}.pt')

    scores = reconstruction_scores(model, X_test, device)

    threshold = sweep_threshold(y_test, scores, beta=2.0)
    y_pred = evaluate(y_test, scores, threshold, f'Predictive LSTM ({BENCHMARK})',
                      out_path=f'{RESULTS_DIR}/lstm_{BENCHMARK}_report.txt')
    plot_results(y_test, y_pred, scores, f'Predictive LSTM ({BENCHMARK})',
                 out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'yahoo', f'lstm_{BENCHMARK}_results.png'))
    plot_loss_curves(train_losses, val_losses,
                     out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'yahoo', f'lstm_{BENCHMARK}_loss_curve.png'))
