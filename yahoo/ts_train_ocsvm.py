...
if __name__ == '__main__':
    ...
    joblib.dump(model, f'{MODELS_DIR}/ocsvm_{BENCHMARK}.pkl')

    scores = -model.decision_function(X_test)

    threshold = sweep_threshold(y_test, scores, beta=2.0)
    y_pred = evaluate(y_test, scores, threshold, f'OC-SVM ({BENCHMARK})',
                      out_path=f'{RESULTS_DIR}/ocsvm_{BENCHMARK}_report.txt')
    plot_results(y_test, y_pred, scores, f'OC-SVM ({BENCHMARK})',
                 out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'yahoo', f'ocsvm_{BENCHMARK}_results.png'))
