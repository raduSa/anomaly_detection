...
class MaskedTransformerAE(nn.Module):
    ...
    def _make_mask(self, batch_size, device):
        n = max(1, int(self.seq_len * self.mask_ratio))
        mask = torch.zeros(batch_size, self.seq_len, dtype=torch.bool, device=device)
        for i in range(batch_size):
            mask[i, torch.randperm(self.seq_len, device=device)[:n]] = True
        return mask
    ...
    def forward(self, x, mask=None):
        ...
        return self.output_proj(self.encoder(tokens))
...
def train(X_train, device):
    ...
    model = MaskedTransformerAE(WINDOW, D_MODEL, NHEAD, NUM_LAYERS, DIM_FF, DROPOUT, MASK_RATIO).to(device)
...
def reconstruction_scores(model, X, device, n_passes=SCORE_PASSES):
    ...
    for _ in range(n_passes):
        ...
        for start in range(0, len(X), BATCH_SIZE):
            ...
            recon = model(batch, mask)
            mask_3d = mask.unsqueeze(-1).float()
            ps.append((((recon - batch) ** 2 * mask_3d).sum(dim=(1, 2)) / mask_3d.sum(dim=(1, 2))).cpu().numpy())
...
if __name__ == '__main__':
    ...
    print(f'Training Masked Transformer AE on {X_train.shape[0]} windows â€¦')
    model, train_losses, val_losses = train(X_train, device)

    torch.save(model.state_dict(), f'{MODELS_DIR}/transformer_ae_{BENCHMARK}.pt')
    ...
    y_pred = evaluate(y_test, scores, threshold, f'Transformer AE ({BENCHMARK})',
                      out_path=f'{RESULTS_DIR}/transformer_ae_{BENCHMARK}_report.txt')
    plot_results(y_test, y_pred, scores, f'Transformer AE ({BENCHMARK})',
                 out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'yahoo', f'transformer_ae_{BENCHMARK}_results.png'))
    plot_loss_curves(train_losses, val_losses,
                     out_path=os.path.join(_ROOT, 'data', 'graphs', 'results', 'yahoo', f'transformer_ae_{BENCHMARK}_loss_curve.png'))
